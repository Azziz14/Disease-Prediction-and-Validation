...
# 1. Refactor downloadReport to remove style manipulation hacks
original_func = """    // Temporarily show the template for capture
...
replacement_func = """    // Trigger direct capture on absolute offscreen DOM structure
    setLoading(true);
...
if original_func in content:
    print("Found downloadReport block. Refactoring logic.")
    content = content.replace(original_func, replacement_func)
else:
    # Fallback matching if space variations exist
    print("WARN: Strict match failed for downloadReport logic. Attempting fuzzy replace...")
    content = re.sub(r'element\.style\.display\s*=\s*\'block\';\s*setLoading\(true\);', 'setLoading(true);', content)
    content = re.sub(r'element\.style\.display\s*=\s*\'none\';\s*setLoading\(false\);', 'setLoading(false);', content)


# 2. Replace the hidden print template EXACT block!
# We will extract from <div id="clinical-print-template" to the closing </div> before <section
# The pristine target spans from Line 330 to Line 429.
start_str = '<div id="clinical-print-template" style={{ display: \'none\', width: \'800px\', padding: \'40px\', backgroundColor: \'#ffffff\', color: \'#000000\', fontFamily: \'Arial, sans-serif\' }}>'
end_str = """        <div style={{ marginTop: '60px', borderTop: '1px solid #e2e8f0', paddingTop: '20px', textAlign: 'center' }}>
...
# Find ending position after start
end_pos = content.find(end_str, start_pos)

if start_pos != -1 and end_pos != -1:
    actual_end = end_pos + len(end_str)
    original_template_block = content[start_pos:actual_end]
    print("Successfully isolated core print template block!")
    
    new_premium_template = """<div id="clinical-print-template" style={{ 
        position: 'absolute', 
        left: '-9999px', 
        top: '-9999px', 
        width: '850px', 
    ...
    content = content.replace(original_template_block, new_premium_template)
    print("Successfully performed EXACT substitution of clinical report template!")
else:
    print("ERROR: Failed to locate absolute boundaries of print template!")
